...
```{.r include="R/divers_functions.R" end-line=6}
```
</details>
...
```{.python include="src/divers_functions.py" end-line=5}
```
</details>
...
```{.r include="R/divers_functions.R" start-line=8}
```
</details>
...
```{.python include="src/divers_functions.py" start-line=9}
```
</details>
...
Reporter les fonctions créées dans cet exercice dans un script `R/divers_functions.R` ({{< fa brands r-project >}}) ou `src/divers_functions.py`.
Enrichir le script `main.R` (resp. `main.py`) pour utiliser celles-ci en créant les _dataframes_
...
```{.r include="intermediates/exo4a.py"}
```
</details>
...
5. Mettre en forme les noms de colonne (`cols_labels`), le titre (`tab_header`), les notes (`tab_source_note`) pour avoir un tableau esthéthique et informatif. Pour les utilisateurs de {{< fa brands r-project >}}, vous pouvez aussi modifier la couleur de la partie supérieure du tableau (`tab_style`).
...
```{.python include="src/tables.py" filename="stc/tables.py"}
```
</details>
...
```{.python include="intermediates/exo4c.R" filename="main.R"}
```
</details>
