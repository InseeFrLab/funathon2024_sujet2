...
Reporter les fonctions créées dans cet exercice dans un script `R/divers_functions.R` ({{< fa brands r-project >}}) ou `src/divers_functions.py` ({{< fa brands python >}}).
Enrichir le script `main.R` (resp. `main.py`) pour utiliser celles-ci en créant les _dataframes_ adéquats (correction ci-dessous).
