...
Cet exercice est, en apparence, un peu long et éloigné du développement d'une application web. Il est toutefois important pour deux raisons: 1/ il permet la construction d'une fonction générique d'import des données qui évite la redondance de code dans les étapes ultérieures; 2/ il vous montre comment organiser des fonctions de manipulation de données qui seront ensuite utilisées par l'application web.
...
de la liste de fichiers `unlist(urls$airports)`[^utilitr-csv] si vous utilisez {{< fa brands r-project >}}, et d'utiliser la fonction `read_csv` du package `pandas` pour lire les données à partir de la liste de fichiers `urls['airports']` si vous utilisez {{< fa brands python >}}.
...
```{.python include="src/import_data.py" start-line=1 end-line=22}
```
...
```{.python include="src/import_data.py" start-line=49 end-line=69}
```
...
1. Reporter la fonction `clean_dataframe` dans un fichier `R/clean_dataframe.R` ({{< fa brands r-project >}}) ou `topinambour/clean_dataframe.py` ({{< fa brands python >}}). Il faudra créer le dossier `R/` ({{< fa brands r-project >}}) ou le dossier `topinambour/` ({{< fa brands python >}}).


2. Reporter les fonctions `import_airport_data`, `import_compagnies_data` et `import_liaisons_data` dans un fichier `R/import_data.R` ({{< fa brands r-project >}}) ou `topinambour/import_data.py` ({{< fa brands python >}}). Dans le cas de Python, le fichier devra commencer par ceci:
...
3. Il est toujours utile de vérifier que nos données sont bien localisées où on les attend. Pour cela, il est pertinent de faire une carte avec un fond de carte contextuel, même si celle-ci n'est pas très soignée. Pour faire ceci, le plus simple est d'utiliser:

- la [fonction `addMarkers`](https://rstudio.github.io/leaflet/articles/markers.html) du package `leaflet` ({{< fa brands r-project >}});
- la [fonction `Map`](https://python-visualization.github.io/folium/latest/reference.html#module-folium.folium) du package `folium` ({{< fa brands python >}}).
