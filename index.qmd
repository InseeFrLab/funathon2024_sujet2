...
et l'interface utilisateur de notre application. Nous reviendrons sur ce concept. Le fichier `global.R` stocke
un certain nombre d'objets utiles à l'application mais qui n'ont pas besoin
d'être recalculés à chaque action sur l'interface graphique. Nous allons progressivement construire ces fichiers pendant les différents exercices. De nombreuses fonctions sont reportées dans les fichiers au sein du dossier `src/`.
...
sur le trafic aérien au niveau d'un aéroport. Une fois qu'elle sera prête, nous pourrons nous pencher sur l'intégration de cette figure dans une application interactive. 

Pour commencer, nous allons créer un fichier fichier `main.R` ({{< fa brands r-project >}}) ou `main.py` ({{< fa brands python >}}) à la racine du dépôt. Ensuite, nous allons ajouter à la fin de ce fichier les deux objets temporaires suivants.
...
complet pour faire cela est développé par `Posit` et
...
Nous allons recréer `ui.R` et `server.R` progressivement afin de comprendre, pas à pas, la logique d'une application.
...
* 3️⃣ On définit des paramètres sur le routage de notre application dans le conteneur. Ces paramètres nous seront utiles ultérieurement, lors du déploiement.
