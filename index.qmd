...
Une démonstration de cette application est disponible à l'adresse <https://rplane-dashboard.kub.sspcloud.fr/>.


Afin de se concentrer sur la démarche, cette application interactive
présentera un nombre limité de fonctionnalités
limitées. L'objectif est de construire
...
mettre en oeuvre des valorisations intéressantes. Pour une phase de construction
d'un prototype, c'est un choix technique intéressant.  
Le partage
de celles-ci, au-delà de partage d'écran, est moins évident car cela 
fait appel à des notions informatiques qui dépassent le champ de compétence
des statisticiens ou _data scientists_. Pour un projet amené à être partagé
à une audience large, sans les compétences techniques pour faire tourner elle-même
le code, ceci est un critère à prendre en considération. D'autres solutions techniques,
plus simple à mettre en oeuvre, comme les sites web statiques
