...
Le premier fichier (`pyproject.toml`) correspond à la liste des _packages_ nécessaires
...
```{.yaml include="pyproject.toml" filename="pyproject.toml"}
```
</details>
...
```{.r include="intermediates/exo3.R" end-line=26}
```
...
* 4️⃣ On restaure l'environnement avec `renv` ({{< fa brands r-project >}}) ou `uv` ({{< fa brands python >}}). De cette manière, on est assuré que l'application aura le même environnement que celui que nous avons prévu lors de la phase de développement.
